...
1.  **Feature Extraction :** Created 2 new Features - Result( Win and Loss ) and Type_of_Voting , these was done by converting all categorical values in numerical and Created Function
    ...
    house_with_New_features$Type_of_Voting1 <- apply(house_with_New_features , 1, get_category)
...
house_with_New_features <- house_with_New_features %>%
  mutate(Result = ifelse(candidatevotes == max(candidatevotes[year == year & 
  state == state & district == district]) &
  sum(year == year & state == state & district == district ) > 1,"Win", "Loss"))
