...
data2016 <- merge(genMaj2016,partyMaj2016) |>
  merge(centroid_labels(regions = c('states')), by = "fips") #adds in data for centroid of each state
...
plot_usmap(data = data2016, values = "winnerByHouseMajority", labels = TRUE) + 
  scale_fill_manual(values = c("REPUBLICAN" = "red", "DEMOCRAT" = "blue"), guide = 'none') 

# +   geom_text(data = data2016, aes(
#     x = x, y = y,
#     label = scales::number(winnerByTotalVotes)
#   ), color = "white")
